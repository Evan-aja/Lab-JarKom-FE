  ...
  return (
    <div className="p-4 lg:p-8 shadow-lg border-2 rounded-xl mb-8">
      <div className="lg:flex">
        <div className="mb-4 lg:mb-0">
          ...
          >
            ...
            {barangs.map((barang, index) => (
              <option key={index} value={barang.id}>
                {barang.nama}
              </option>
            ))}
          </select>
        ...
        <div className="flex justify-between lg:w-full">
          <Input
            ...
            onChange={onChange}
            type="number"
            className="mr-4 w-full lg:w-auto"
            placeholder={t("form:amount_placeholder")!}
            max={barang ? barang.jumlah : 0}
            bordered
            required
          />
          <Button
      ...
      {barang && (
        <div>
          <div className="mb-6">
            <h5 className="text-baseDark font-bold text-xl mt-6 mb-3">
              {t("services:borrow.item_name")}
            </h5>
            {barang?.nama}
          </div>
          <div className="mb-6">
            <h5 className="text-baseDark font-bold text-xl mt-6 mb-3">
              {t("services:borrow.item_description")}
            </h5>
            {barang?.deskripsi}
          </div>
          <div>
            <h5 className="text-baseDark font-bold text-xl mt-6 mb-3">
              {t("services:borrow.item_stock")}
            </h5>
            {barang?.jumlah}
          </div>
        </div>
